...
logstash_directory = "logstash-1.5.2" 
logstash_archive = logstash_directory + ".tar.gz"
logstash_server = "https://download.elastic.co/logstash/logstash"
...
def download_and_untargz(archive, directory_url, directory):
    if not os.path.exists(archive):
        os.system('wget ' + directory_url + "/" + archive)
    if not os.path.exists(directory):
        os.system('tar xzf ' + archive)
def logstash_download():
    download_and_untargz(logstash_archive, logstash_server, logstash_directory)
def tail():
    kafka_download()
    logstash_download()
    os.chdir("src")
    os.system('../' + logstash_directory + '/bin/logstash agent -f ../conf/logstash.conf')
def clean():
    """ Cleanups file generated by this script """
    autoclean()
def kafka_download():
    """ Downloads kafka binary distribution archive and uncompresses it """
    download_and_untargz(kafka_archive, kafka_server + kafka_version, kafka_directory)
def zookeeper_start():
